...
export const deleteClientApi = async (props: Props) => {
    try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/authorities/${props.id}`, { method: 'DELETE' });
...
export const createOrUpdateClientApi = async (props: Props) => {
    try {
        const url = props.id ? `${process.env.NEXT_PUBLIC_HOST}/api/authorities/${props.id}` : `${process.env.NEXT_PUBLIC_HOST}/api/tags`;
