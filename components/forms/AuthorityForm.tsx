...
import { CreateAuthorityType, CreateAuthoritySchema } from '@/validations/authority-schema';
...
const AuthorityForm = ({ id, name }: Props) => {
    ...
    const onSubmit: SubmitHandler<AuthorityType> = async (data) => {
        ...
        const url = id ? `${process.env.NEXT_PUBLIC_HOST}/api/authorities/${id}` : `${process.env.NEXT_PUBLIC_HOST}/api/authorities`;
        ...
        if (res.status === 200) {
            ...
            router.push(`${process.env.NEXT_PUBLIC_HOST}/usermanagement/authority`);
        } else {
    ...
    return (
        <>
            <div className='flex-none flex justify-start items-center text-sm gap-2 ps-7 py-4 uppercase mt-[50px] lg:mt-0'>
                ...
                <Link href='/usermanagement/authority'>Authority</Link>
                <ArrowRightIcon className='w-3 h-3' />
                <span>Form</span>
            </div>
            <FormContainer>
                <div className='flex justify-center items-center w-full max-w-md sm:bg-slate-50 sm:border border-slate-300 rounded-lg mx-4 my-8 sm:mx-0 px-5 py-14 '>
                    <form
                        ...
                        <div className="mt-5 text-center flex gap-1">
                            <button 
                                onClick={() => router.push(`${process.env.NEXT_PUBLIC_HOST}/usermanagement/authority`)}
                                type='button'
